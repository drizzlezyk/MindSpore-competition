...
def parse_args():
    ...
    parser.add_argument('--imdb_path',
                        type=str, default='C:\\datasets\\aclImdb_v1.tar.gz',
                        help='imbd path')
...
def load_data(data_path="./dataset/train.csv"):
    dataset = ds.CSVDataset(dataset_files=data_path, column_names=["review", "label"])
...
def load_process_data():
    data_train = ds.GeneratorDataset(AmazonData("./dataset/test1.csv"),
    ...
    data_train = ds.GeneratorDataset(data_train, column_names=["review", "label"])
...
def test_predict(vocab, embeddings):
    ...
    res = []
    with open("C:\\datasets\\amazon_for_us\\test.csv", "r") as test_file:
